...
const getborderStyle = (isFirst) => {
  const style: any = { borderColor: colors.black50, borderBottomWidth: 1, borderBottomStyle: 'solid', }
  if (isFirst) {
...
const ContentExpandable: React.SFC<ContentExpandable> = ({ id, title, content, isFirst }) => {
  ...
  return (
    <Div _width='100%' style={getborderStyle(isFirst)}>
      <Div _direction='row' _width='100%' justify='space-between' padd='32px 0px'>
        <Font type='Futura' weight='bold' size='18px'>{title}</Font>
      ...
      <Animated.ScrollView style={{width:'100%', height:height, overflow:"scroll"}}>
      <HTML html={content} />
      </Animated.ScrollView>
    </Div>
  )
