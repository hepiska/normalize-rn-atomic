...
const renderItem = ({ item }) => {
  return (
    <ProductItem product={item} style={{ width: ((width / 2) - 16 - 8), marginRight: 16 }} />
  )
}

const ProductGroupList = ({ products, title, style, onShowAll }: ProductGroupType) => {

  return (
    <Div _width='100%' style={style} align='flex-start'>
      <Div _margin='16px 0px' _flex='1' _width='100%' justify='space-between' align="stretch" _direction='row'>
        <Font size={18} type='Futura' weight='bold'>{title}</Font>
        {
          onShowAll && (<PressAbbleDiv>
            <Div _direction='row'>
              <Font size={14} color={colors.blue50}>Show All</Font>
              <Icon name='chevron-right' color={colors.blue50} style={{ marginLeft: 8 }}></Icon>
            </Div>
          </PressAbbleDiv>)
        }
